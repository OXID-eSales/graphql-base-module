...
# Generate the documentation from account module,
# as we want to generate schema for it as well.
cd ../
git clone https://github.com/OXID-eSales/graphql-account-module.git
cd graphql-account-module
...
echo -n "    - { resource:" >> $MODULE_DIR/var/configuration/configurable_services.yaml
echo  " SET_PATH_HERE_C }" >> $MODULE_DIR/var/configuration/configurable_services.yaml
sed -i "s|\SET_PATH_HERE_A|\'$MODULE_DIR/services.yaml'|" $MODULE_DIR/var/configuration/configurable_services.yaml
sed -i "s|\SET_PATH_HERE_B|\'$MODULE_DIR/vendor/oxid-esales/graphql-catalogue/services.yaml'|" $MODULE_DIR/var/configuration/configurable_services.yaml
sed -i "s|\SET_PATH_HERE_C|\'$MODULE_DIR/vendor/oxid-esales/graphql-base/services.yaml'|" $MODULE_DIR/var/configuration/configurable_services.yaml
